...
<div class="body3">
    <div class="main">
        ...
        <article id="content">
            <div class="wrapper">
                <section class="col1">
                    <h2 class="under">Mis Turnos:</h2>
                    <%
                        session = request.getSession();
                        String email = request.getRemoteUser();
                        Paciente p = (Paciente)UsuarioDAO.getUsuarioPorEmail(email);
                        List<Turno> turnos = p.getTurnos();
                        if(turnos.isEmpty()) {
                    %>
                    <td>Usted no tiene turnos reservados</td>
                    <form id="Volver" method="post">
                        <span><a href="paciente.jsp" class="button">Volver</a></span>
                    </form>
                    <%
                    } else {
                    %>
                    <td>Seleccione los turnos que desea cancelar</td>
                    <form method="post" action="CancelarTurnosServlet">
                        <ol>
                            <td>
                                <%
                                    for(int i=0;i<turnos.size();i++) {
                                        Turno t = turnos.get(i);
                                        String a = t.getDate().toString();
                                        String b = (a.substring(0, 10)).concat("T").concat(a.substring(11, a.length()));
                                %>
                                <input type="checkbox" name="eliminar" value=<%=b%>/><%=a%> con el doctor <%=t.getNombreDoctor()%><br>
                            </td>
                        </ol>
                        <%
                            }
                        %>
                        <input type="submit" class="button" value="Eliminar turnos" align="center">
                    </form>
                    <%
                        }
                    %>
